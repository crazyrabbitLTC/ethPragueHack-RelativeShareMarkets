...
async function testDeployedContracts() {
  ...
  const deploymentPath = path.join(__dirname, '../contracts/deployments/arbitrum.json');
  ...
  try {
    ...
    try {
      ...
      console.log(`${colors.yellow}Note: Minting may require special permissions${colors.reset}`);
    }
    ...
    // Only continue if they have USDC
    if (aliceUSDC > 0 && bobUSDC > 0) {
      ...
      const newETHPrice = ethers.parseUnits('3300', 18);
      ...
      console.log(`${colors.yellow}⚠️  Skipping trading tests - wallets need USDC first${colors.reset}`);
      console.log(`Please mint USDC to Alice and Bob addresses manually`);
    }
